...
Our scenarios are thus defined by the probability of AB identification at the peak of equal ancestry and the degree to which this probability declines as ancestry moves toward one group or the other. To determine the appropriate parameters for the multinomial model that determines the probabilities, we determine five values which allow us to fit the model parameters. First, we determine what proportion of individuals with 50/50 ancestry from groups A and B (e.g. "first generation biracial individuals") will identify as AB, or mixed. We then consider how much this proportion would decline in the succeeding second generation (75/25) and third generation (87.5/12.5). These three numbers give us five points along the curve predicting  the probability of AB ancestry as a function of the proportion of A ancestry that an individual possesses. We then need to also determine the probability of A and B assignment. We assume that at 50/50, a person who does not identify as AB will be equally likely to identify as A or B. We then use two additional parameters determining the ratio of single group ancestry for the group with more ancestry vs less ancestry at the second and third generation points. For example, a value of 4 for second generation would mean that individuals with 75% group A ancestry are 4 times more likely to identify as group A rather than group B in cases where they don't identify as AB. These five values allow us to fit a multinomial logit model with high precision to handle the many cases that will not correspond to first, second, and third generation individuals over the course of the simulation.

 We rely on recent empirical work to determine parameters for our scenario. Based on the work of Saperstein and Morning and Xu et. al, we identify a "high" peak value of 75% and a "low" peak value of 40%. We also identify a "resilient" pattern of AB identification in which the declines are XX and XX for second and third generation, respectively, and a "decay" pattern of AB identification in which the declines are XX and XX for second and third generation, respectively. 

Determining the two final ratios is more difficult based on empirical work. We use a ratio of 4 to 1 in the second generation and 20 to 1 in the third generation. Bratter (CITE) examines the identification choices made on behalf of children where one parent is multiracial, which is likely somewhat consistent with the second generation, and finds even more extreme ratios. 

@fig-scenarios shows the assignment probabilities across these four scenarios.
